...
export default async function apiUsingExample(): Promise<void> {
  ...
  const apiServices: ApiServices = ApiServices.getInstance().setNewApiClient('kirill@example.com', 'myPassword123');
  ...
  const responceCreateCustomer: ClientResponse<CustomerSignInResult> | void = await apiServices.createCustomer(body);
